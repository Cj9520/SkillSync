        ...
        <section className="feedback-section bg-[url('/images/bg-small.svg')] bg-cover h-[100vh] sticky top-0 items-center justify-center">
          {imageUrl && resumeUrl && (
            <div className="animate-in fade-in duration-1000 max-sm:m-0 h-[90%] max-2xl:h-fit w-fit flex flex-col items-center">
              {/* Resume display - either as image or PDF depending on isPdfDirect flag */}
              <div className="bg-white rounded-lg p-4 shadow-lg">
                {isPdfDirect ? (
                  <iframe
                    src={resumeUrl}
                    className="w-[600px] h-[800px] max-w-full max-h-[calc(100vh-150px)]"
                    title="Resume PDF"
                    style={{ border: 'none' }}
                  ></iframe>
                ) : (
                  <img
                    src={imageUrl}
                    alt="Resume Preview"
                    className="max-h-[calc(100vh-120px)] max-w-[600px] object-contain"
                  />
                )}
              </div>
