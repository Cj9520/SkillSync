...
const Upload = () => {
    ...
    const handleAnalyze = async ({ companyName, jobTitle, jobDescription, file }: { companyName: string, jobTitle: string, jobDescription: string, file: File  }) => {
        ...
        try {
            ...
            // Set a timeout for PDF conversion
            const conversionTimeout = 3000; // 3 seconds max
            
            // Try to convert using simplified method with timeout
            let imageFile;
            let directPdfUsed = false;
            
            try {
                // Create a promise that rejects after the timeout
                const timeoutPromise = new Promise((_, reject) => {
                    setTimeout(() => reject(new Error('PDF conversion timeout')), conversionTimeout);
                });
                ...
                if (result.file) {
                    // If the result file is a PDF (not a converted image), mark as using direct PDF
                    if (result.file.type === 'application/pdf') {
                        directPdfUsed = true;
                        console.log('Using PDF directly as preview');
                    }
                    imageFile = result.file;
                } else {
                    throw new Error('Conversion failed, using fallback');
                }
            } catch (conversionError) {
                console.warn('PDF conversion failed or timed out, using direct PDF', conversionError);
                directPdfUsed = true;
                
                // Use the PDF file directly
                imageFile = file;
            }
        ...
        const data = {
            ...
            feedback: '',
            isPdfDirect: directPdfUsed  // Flag to indicate if we're using direct PDF viewing
        }
